...
// const OwnedWordsNft = artifacts.require("OwnedWords");
...
//     this.token = await OwnedWordsNft.new({ from: deployer });
//   });
...
//     expect(await this.token.get_version().to.equal("1.0.0"));
//   });
...
//       const listOfOwnedTokens = await this.token.get_tokens({
//         from: other,
//       });
...
//       const tokenIdForPost = await this.token.get_token_id_for_post(
//         mint_args[0],
//         mint_args[1],
//         mint_args[4]
//       );
...
//       await expectRevert(
//         this.token.get_token_id_for_post(
//           another_mint_args[0],
//           another_mint_args[1],
//           another_mint_args[4]
//         ),
...
contract("OwnedWords", function (accounts) {
  ...
  describe("contract version", function () {
    it.skip("should return the contract version", async function () {
      expect(await this.token.get_version({ from: other })).to.equal("1.0.0");
    });
  });
});
