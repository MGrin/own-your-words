...
const OwnedWordsNft = artifacts.require("SocialNetworkOwnership");

contract("SocialNetworkOwnership", function (accounts) {
  const [deployer, other] = accounts;

  const name = "SocialNetworkOwnership";
  const symbol = "SNO";
  ...
  describe("minting", function () {
    it("deployer can mint tokens", async function () {
      ...
      const receipt = await this.token.mint(other, "test_1234", {
        from: deployer,
      });
      ...
      expect(await this.token.tokenURI(tokenId)).to.equal("DO IT!");
    });

    it("other accounts cannot mint tokens", async function () {
      await expectRevert(
        this.token.mint(other, "test_1234", { from: other }),
